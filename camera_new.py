...
def save_zero_point(x, y):
    with open(ZERO_POINT_FILE, "w") as f:
        json.dump({"zero_x": x, "zero_y": y}, f, indent=4)
    print(f"Saved zero point: x={x}, y={y}")
...
if zero_x is None or zero_y is None:
    print("Error: zero point not found.")
    exit()
...
def mouse_callback(event, x, y, flags, param):
    global clicked_point, zero_x_mm, zero_y_mm, zero_x, zero_y
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        zero_x, zero_y = x, y
        zero_x_mm, zero_y_mm = pixel_to_world_homography(zero_x, zero_y, H)
        print(f"Updated zero point in mm: X={zero_x_mm:.2f}, Y={zero_y_mm:.2f}")
...
while cap.isOpened():
    ...
    if clicked_point:
        cv2.circle(frame, clicked_point, 5, (255, 255, 0), -1)
